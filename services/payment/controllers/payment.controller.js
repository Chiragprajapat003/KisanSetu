...
exports.createOrder = asyncHandler(async (req, res, next) => {
    ...
    if (paymentMethod === 'online') {
        let razorpayOrder;
        const isMockRazorpay = !process.env.RAZORPAY_KEY_ID ||
            process.env.RAZORPAY_KEY_ID === 'your_razorpay_key_id' ||
            process.env.RAZORPAY_KEY_ID.includes('placeholder');

        if (isMockRazorpay) {
            console.log('ℹ️ Using mock Razorpay order for development/testing');
            razorpayOrder = {
                id: `order_mock_${order._id.toString().slice(-6)}_${Date.now()}`,
            ...
            try {
                ...
                console.warn('⚠️ Razorpay orders.create failed (e.g. invalid credentials), falling back to test mock:', rzpErr.message);
                razorpayOrder = {
                    id: `order_mock_${order._id.toString().slice(-6)}_${Date.now()}`,
                    amount: Math.round(order.totalPrice * 100),
                    currency: 'INR'
